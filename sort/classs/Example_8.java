...
public class Example_8 {
	 ...
	 * 在每组中再进行直接插入排序。当增量减到1时，进行直接插入排序后，排序完成。 （2）实例： （3）用java实现
	 */
	 ...
	 * 当且仅当满足（hi>=h2i,hi>=2i+1）或（hi<=h2i,hi<=2i+1）
	 * (i=1,2,...,n/2)时称之为堆。在这里只讨论满足前者条件的堆
	 ...
	 * 。然后对前面(n-1)个数重新调整使之成为堆。依此类推
	 * ，直到只有两个节点的堆，并对它们作交换，最后得到有n个节点的有序序列。从算法描述来看，堆排序需要两个过程
	 * ，一是建立堆，二是堆顶与堆的最后一个元素交换位置。所以堆排序有两个函数组成。一是建堆的渗透函数，二是反复调用渗透函数实现排序的函数。 （2）实例：
	 * 初始序列：46,79,56,38,40,84 建堆： 交换，从堆中踢出最大数 依次类推：最后堆中剩余的最后两个结点交换，踢出一个，排序完成。
	 * （3）用java实现
	 */
	...
	class quickSort {
		...
		public int getMiddle(int[] list, int low, int high) {
			...
			while (low < high) {
				// 从右向左找小于tmp的数来填list[low]
				while (low < high && list[high] >= tmp) {
					high--;
				}
				list[low] = list[high];
				// 从左向右找大于或等于tmp的数来填list[high]
				while (low < high && list[low] <= tmp) {
					low++;
				}
				list[high] = list[low];
			}
			list[low] = tmp; // 中轴记录到中间位置
			return low; // 返回中轴的位置
		}
	}
	...
	/**
	 * 8、基数排序 （1）基本思想：将所有待比较数值（正整数）统一为同样的数位长度，数位较短的数前面补零。然后，从最低位开始，依次进行一次排序。
	 * 这样从最低位排序一直到最高位排序完成以后,数列就变成一个有序序列。 （2）实例：
	 * 
	 * 
	 * （3）用java实现
	 */
